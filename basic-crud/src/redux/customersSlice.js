  ...
  initialState: {
    data: [
      ...
      },
      {
        ...
        "First Name": "Ryan",
        "Last Name": "Evans",
        "Phone Number": 4441112222,
        Email: "ryan.e@example.com",
...
export const selectFiltersData = (state) => {
  ...
  const confirmedFilter = Array.isArray(selectedFilters.confirmed)
    ? selectedFilters.confirmed[0].value
    : selectedFilters.confirmed;
  const activeOrdersFilter = Array.isArray(selectedFilters.activeOrders)
    ? selectedFilters.activeOrders[0].value
    : selectedFilters.activeOrders;

  const filteredData = data.filter((row) => {
    const matchConfirmed =
      confirmedFilter === "All" ||
      row["Confirmed"].toString() === confirmedFilter;
    const matchActiveOrders =
      activeOrdersFilter === "All" ||
      row["Active Order"].toString() === activeOrdersFilter;
