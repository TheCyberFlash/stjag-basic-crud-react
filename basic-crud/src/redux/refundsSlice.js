...
export const selectFiltersData = (state) => {
  ...
  const refStatusFilter = Array.isArray(selectedFilters.refStatus)
    ? selectedFilters.refStatus[0].value
    : selectedFilters.refStatus;
  const partialFilter = Array.isArray(selectedFilters.partial)
    ? selectedFilters.partial[0].value
    : selectedFilters.partial;
  const reasonFilter = Array.isArray(selectedFilters.reason)
    ? selectedFilters.reason[0].value
    : selectedFilters.reason;

  const filteredData = data.filter((row) => {
    const matchRefStatus =
      refStatusFilter === "All" || row["Ref Status"] === refStatusFilter;
    const matchPartial =
      partialFilter === "All" || row["Partial"].toString() === partialFilter;
    const matchReason =
